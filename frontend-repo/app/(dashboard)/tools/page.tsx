...
export default function ToolsPage() {
    ...
    const filtered = tools.filter((t) => {
        ...
        const matchStatus =
            statusFilter === "all" || String(t.status) === statusFilter;
    ...
    const handleTest = async (tool: any) => {
        ...
        try {
            addToast({
                type: "success",
                title: `${tool.name} — Test passed`,
                message: `Response in ${Math.floor(Math.random() * 50 + 10)}ms`,
            });
        } finally {
            setTestingId(null);
        }
    ...
    const handleSync = async (tool: any) => {
        ...
        try {
            ...
            addToast({
                type: "success",
                title: `${tool.name} synced`,
                message: "n8n workflow up to date.",
            });
        } finally {
            setSyncingId(null);
        }
    ...
    const columns: TableColumn<any>[] = [
        ...
        },
        {
            ...
            label: "Endpoint",
            render: (v) => (
                <code className={styles.endpoint}>{String(v || "—")}</code>
            ),
        },
        {
            key: "category",
            ...
            sortable: true,
            render: (v) => (
                <span className={styles.workflowPill}>{String(v || "—")}</span>
            ),
        },
        {
            key: "status",
            ...
            width: "100px",
            render: (v) => (
                <StatusBadge status={String(v).toLowerCase()} size="sm" />
            ),
        },
        {
            key: "n8nType",
