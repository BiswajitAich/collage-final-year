...
import {
  ArrowLeft, Edit3, Copy, Clock, CheckCircle2, Activity
} from 'lucide-react';
import { getWorkflowById } from '../action';
import { useUIStore } from '@/stores';
import { StatusBadge, LoadingSkeleton, ErrorState } from '@/components/ui/UIComponents';
import { Button } from '@/components/ui/Button';
import { WorkflowGraph } from '@/components/workflow/WorkflowGraph';
import {
  formatNumber, formatPercent, formatLatency, timeAgo
} from '@/lib/utils';
...
export default function WorkflowDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!data) { setError('Workflow not found'); setIsLoading(false); return; }
      setWf(data);
      setIsLoading(false);
    }
    load();
        ...
        <div className={styles.leftCol}>
          ...
          <div className={styles.graphCard}>
            ...
            <div className={styles.graphWrap}>
              <WorkflowGraph nodes={wf.workflowJson?.graph?.nodes ?? []} edges={wf.workflowJson?.graph?.edges ?? []} />
            </div>
          </div>
        </div>
