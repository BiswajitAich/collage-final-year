...
import { generateWorkFlow } from "../generateWorkFlow.action";
import { addWorkflowToN8n } from "../addWorkflowToN8n.action";
...
export default function NewWorkflowPage({
    ...
    const [generatedWorkflow, setGeneratedWorkflow] =
        useState<WorkflowGraphData | null>(workflow ?? null);
    ...
    const onGenerate: SubmitHandler<WorkflowGenerationFormData> = async (
        data: WorkflowGenerationFormData,
    ) => {
        console.log("[UI CHECKPOINT] onGenerate started", {
            schemaId,
            capabilityId,
        });
        ...
        try {
            ...
            console.log(
                "[UI CHECKPOINT] Workflow generated, DB id:",
                wf.id,
                "graph id:",
                (wf as any).id,
            );
    ...
    const handleApprove = async () => {
        ...
        console.log(
            "[UI CHECKPOINT] savedWorkflowId (DB id):",
            savedWorkflowId,
        );
        console.log(
            "[UI CHECKPOINT] generatedWorkflow?.id (graph id):",
            (generatedWorkflow as any)?.id,
        );
        ...
        addToast({
            ...
            message: "Compiling and creating workflow.",
        });
        try {
            ...
            addToast({
                type: "success",
                title: "Deployed!",
                message: "Workflow approved & saved to n8n.",
            });
        } catch (error) {
                                        ...
                                        {GENERATION_MODE_OPTIONS.map((mode) => (
                                            ...
                                            >
                                                <input
                                                    ...
                                                    disabled={
                                                        !(mode.value === "AI")
                                                    } // dissable others for now, except ai creation !!!
                                                    {...register(
                                                        "generationMode",
                                                    )}
                                    ...
                                    {(
                                        [
                                            ...
                                            },
                                            {
                                                name: "isReadOnly",
                                                label: "Read Only",
                                                desc: "GET operations only",
                                            },
                                            {
                                                name: "enableCRUD",
                                                label: "Full CRUD",
                                                desc: "All HTTP methods",
                                            },
                                            {
                                                name: "strictValidation",
                                                label: "Strict Validation",
                                                desc: "Reject invalid payloads",
                                            },
                                            {
                    ...
                    {isGenerating ? (
                        <div className={styles.generatingState}>
                            ...
                            <p className={styles.generatingDesc}>
                                Analyzing entities, inferring business logic,
                                building node graph…
                            </p>
                            ...
                            <div className={styles.generatedHeader}>
                                ...
                                <div className={styles.generatedMeta}>
                                    <span className={styles.metaChip}>
                                        {generatedWorkflow.httpMethod}
                                    </span>
                                    <span className={styles.metaChip}>
                                        {generatedWorkflow.endpointType}
                                    </span>
                                    <span className={styles.metaChip}>
                                        {generatedWorkflow?.graph?.nodes.length}{" "}
                                        nodes
                                    </span>
                                    <span className={styles.metaChip}>
                                        {generatedWorkflow?.graph?.edges.length}{" "}
                                        connections
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className={styles.capabilityPreview}>
                            ...
                            <div className={styles.capabilityMeta}>
                                <div className={styles.capabilityMetaItem}>
                                    <span className={styles.capabilityMetaKey}>
                                        Method
                                    </span>
                            ...
                            <div className={styles.capabilityPrompt}>
                                <Zap size={13} /> Form is pre-filled — hit{" "}
                                <strong>Generate Workflow</strong> to build the
                                graph.
                            </div>
                        </div>
                    ) : (
                        <div className={styles.previewEmpty}>
                            ...
                            <h3 className={styles.previewEmptyTitle}>
                                Ready to generate
                            </h3>
