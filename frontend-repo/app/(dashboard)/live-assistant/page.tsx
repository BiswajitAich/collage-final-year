...
export default function LiveAssistantPage() {
  ...
  async function startCall() {
    ...
    try {
      ...
      if (callMode === 'mobile') {
        const dialRes = await fetch(`${FASTAPI_BASE_URL}/rooms/outbound-call`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (!dialRes.ok) {
          ...
          throw new Error(text || `Failed to start mobile call (${dialRes.status})`);
        }
      }
    } catch (err) {
  ...
  return (
    <main className={styles.page}>
      ...
      <button className={styles.endBtn}
        onClick={() => setCallMode((prev) => { return prev === "browser" ? "mobile" : "browser" })}
      >
        {callMode === "mobile" ? "Call through Browser" : "Call through Mobile"}
      </button>
    </main>
  );
}